...
void Logm::command_c() {
    ...
    cat.erase(0,1);
    to_lower_case(cat);
    if (c_map.find(cat) == c_map.end()) std::cout << "Category search: 0 entries found" << '\n';
    else std::cout << "Category search: " << c_map[cat].size() << " entries found" << '\n';
}
...
void Logm::command_k() {
    ...
    if (k_map.find(sub) == k_map.end()) {
        std::cout << "Keyword search: 0 entries found" << '\n';
        return;
    }
    prev_ind.resize(k_map[sub].size());
    copy(k_map[sub].begin(), k_map[sub].end(), prev_ind.begin());
    while (!isalnum(key_mes[begin])) {
        begin ++;
        end ++;
    }
    ...
    while (end < key_mes.length()) {
        ...
        auto itt = std::set_intersection(prev_ind.begin(), prev_ind.end(), k_map[sub].begin(),
                                        k_map[sub].end(), prev_ind.begin());
...
void Logm::command_r() {
    ...
    if (!searched) throw commandError();
        ...
        if (c_map.find(cat) != c_map.end()) for (int &ind: c_map[cat]) excerpt.push_back(ind);
    }
...
void Logm::command_s() {
    if (excerpt.empty()) std::cout << "excerpt list sorted" << '\n'
        ...
        int index = excerpt.front();
        std::cout << "excerpt list sorted" << '\n'
                  << "previous ordering:" << '\n'
                  << "0|"
                  << main_log[index].index << "|"
                  << main_log[index].time << "|"
                  << main_log[index].cat << "|"
                  << main_log[index].mes << '\n';
        index = excerpt.back();
...
void Logm::command_g() {
    ...
    else if (cated) for (int &ind: c_map[cat])  std::cout << main_log[ind].index << "|"
                                                          << main_log[ind].time << "|"
                                                          << main_log[ind].cat << "|"
                                                          << main_log[ind].mes << '\n';
    else for (int &ind: prev_ind)  std::cout << main_log[ind].index << "|"
                                             << main_log[ind].time << "|"
                                             << main_log[ind].cat << "|"
                                             << main_log[ind].mes << '\n';
}
...
void Logm::command_p() {
    int count = 0;
    for(int &ind: excerpt)  std::cout << count ++ << "|"
                                      << main_log[ind].index << "|"
                                      << main_log[ind].time << "|"
                                      << main_log[ind].cat << "|"
                                      << main_log[ind].mes << '\n';
...
void Logm::cleanup_k(std::string &word, int cur_ind) {
    ...
    while (end < word.length()) {
        ...
        if(k_map[sub].empty() || k_map[sub].back() != cur_ind) k_map[sub].push_back(cur_ind);
        begin = end;
